...
public class VectorPlugin extends Plugin implements ScriptPlugin {
	@Override
					...
					public ScoreScript newInstance(LeafReaderContext context) throws IOException {
						...
						return new ScoreScript(p, lookup, context) {
							...
							public double execute() {
								...
								final DoubleBuffer doubleBuffer = ByteBuffer.wrap(bytes, position, docVectorLength)
										.asDoubleBuffer();

								final double[] docVector_input = new double[inputVectorSize];
								doubleBuffer.get(docVector_input);
								...
								for (int i = 0; i < inputVectorSize; i++) {
									...
									score += docVector_input[i] * inputVector[i];

									if (cosine) {
										docVectorNorm += Math.pow(docVector_input[i], 2.0);
									}
								}
